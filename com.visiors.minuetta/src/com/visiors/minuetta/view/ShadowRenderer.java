...
public class ShadowRenderer {
	...
	public static BufferedImage makeShadow(BufferedImage source, int size, Color color) {
		...
		final ShadowRenderer sr = new ShadowRenderer(size, 0.3f, color);
		final BufferedImage imgShadow = sr.createShadow(source);
	...
	public static BufferedImage makeShadow(BufferedImage source) {
		...
		final ShadowRenderer sr = new ShadowRenderer(shadowSize, 0.2f, Color.darkGray);
		final BufferedImage imgShadow = sr.createShadow(source);
		imgShadow.getGraphics().drawImage(source, 0, 0, null);
		return imgShadow;
	}
	 ...
	 * renderer provides the following default values:
	 * <ul>
	...
	public BufferedImage createShadow(final BufferedImage image) {
		...
		final int shadowRgb = color.getRGB() & 0x00FFFFFF;
		...
		final int[] dstBuffer = new int[dstWidth * dstHeight];
		final int[] srcBuffer = new int[srcWidth * srcHeight];
		...
		final float vSumDivider = opacity / shadowSize;

		final int[] hSumLookup = new int[256 * shadowSize];
		for (int i = 0; i < hSumLookup.length; i++) {
			hSumLookup[i] = (int) (i * hSumDivider);
		}

		final int[] vSumLookup = new int[256 * shadowSize];
		...
		for (int srcY = 0, dstOffset = left * dstWidth; srcY < srcHeight; srcY++) {
			...
			for (int srcX = 0; srcX < srcWidth; srcX++) {
				...
				dstBuffer[dstOffset++] = a << 24; // store the alpha value only
												  // the shadow color will be
												  // added in the next pass

				aSum -= aHistory[historyIdx]; // substract the oldest pixel from
											  // the sum
		...
		for (int x = 0, bufferOffset = 0; x < dstWidth; x++, bufferOffset = x) {
			...
			for (int y = 0; y < yStop; y++, bufferOffset += dstWidth) {
				...
				dstBuffer[bufferOffset] = a << 24 | shadowRgb; // store alpha
															   // value + shadow
															   // color
