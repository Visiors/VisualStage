package com.visiors.visualstage.generics.renderer.resource.svg;
...
import com.visiors.visualstage.generics.attribute.DefaultPropertyList;
import com.visiors.visualstage.generics.attribute.DefaultPropertyUnit;
...
public class SVGDefinition {
	...
	private static String[] symbolAttributeTag = new String[] { "x", "y", "width", "height",
			"viewBox", "transform", "preserveAspectRatio" };

	public SVGDefinition(PropertyList pl) {
	...
	private void init(String svgContent) {
		this.definition = svgContent;
		this.width = getWidth(definition);
		this.height = getHeight(definition);
		this.ratio = keepAspectRatio(definition);
		this.keepRatio = ratio != 0;
	}
	...
	private boolean documentAttribute(Property p) {
		...
		for (String element : symbolAttributeTag) {
	...
	private int findSVGTag(StringBuffer svg) {
		...
		for (int i = 0; i < svg.length() - 5; i++) {
			if (Character.toLowerCase(svg.charAt(i)) == 's'
					&& Character.toLowerCase(svg.charAt(i + 1)) == 'v'
