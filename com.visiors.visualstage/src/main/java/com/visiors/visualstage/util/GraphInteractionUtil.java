...
public class GraphInteractionUtil {
	...
	public static VisualNode getFirstHitNodeAt(VisualGraph rootGraphView, Point pt) {
		...
		final List<VisualGraphObject> hitObjects = getHitObjects(rootGraphView, pt, false, false, true);
	...
	public static VisualEdge getFirstHitEdgeAt(VisualGraph rootGraphView, Point pt) {
		...
		final List<VisualGraphObject> hitObjects = getHitObjects(rootGraphView, pt, true, true, false);
		if (hitObjects.size() > 0) {
	...
	public static VisualGraph getFirstHitGroupAt(VisualGraph rootGraphView, Point pt) {
		...
		final List<VisualGraphObject> hitObjects = getHitObjects(rootGraphView, pt, false, true, true);
		if (hitObjects.size() > 0) {
	...
	public static VisualGraph getLastGroupHitByObjects(VisualGraph rootGraphView, List<VisualGraphObject> objects) {
		...
		for (final VisualGraphObject vgo : objects) {
			final VisualGraph gv = getLastGroupHitByObjects(rootGraphView, vgo);
			if (gv != null) {
				return gv;
			}
		}
	...
	public static VisualGraph getLastGroupHitByObjects(VisualGraph rootGraphView, VisualGraphObject vgo) {
		...
		final List<VisualGraphObject> allHitObjects = new ArrayList<VisualGraphObject>();

		hitObjects = getHitObjects(rootGraphView, new Point(r.x, r.y), false, true, true);
		for (final VisualGraphObject gv : hitObjects) {
		...
		if (allHitObjects.size() != 0) {
			for (final VisualGraphObject gv : allHitObjects) {
	...
	public static VisualGraphObject getFirstHitObjectAt(VisualGraph rootGraphView, Point pt) {
		...
		final List<VisualGraphObject> hitObjects = getHitObjects(rootGraphView, pt, false, false, false);
		if (hitObjects.size() > 0) {
			for (final VisualGraphObject obj : hitObjects) {
				if (obj.isSelected()) {
					return obj;
				}
			}
	...
	public static VisualGraphObject getClosestObject(VisualGraph visualGraph, VisualGraphObject refObject,
		...
		if (!ignoreNodes || !ignoreGroups) {
			...
			final Point cpt = new Point(bref.x + bref.width / 2, bref.y + bref.height / 2);
	...
	public static VisualGraphObject getClosestNode(VisualGraph visualGraph, VisualGraphObject refObject,
			int maxDistance, boolean ignoreOverlaps, Port[] posrts) {

		double minDist = Double.MAX_VALUE;
		VisualGraphObject bestmatch = null;
		Rectangle b;
		double r;
		final Rectangle bref = refObject.getBounds();
		...
		for (final VisualNode node : nodes) {
			...
			if (node instanceof VisualGraph) {
				final VisualGraphObject no = getClosestNode((VisualGraph) node, refObject, maxDistance, ignoreOverlaps,
						posrts);
				if (no != null) {
					return no;
				}
			...
			final double alpha = getAngle(bref, b);
			final Port pRef = getAvailableport((VisualNode) refObject, alpha);
			if (pRef == null) {
				continue;
			}
			// find the port on the opposite side
			final int beta = portMidAngle(pRef);
			final Port pNode = getAvailableport(node, beta + 180);

			if (pNode == null) {
				continue;
			}

			final Point pt1 = pRef.getPosition();
			final Point pt2 = pNode.getPosition();
	...
	private static int portMidAngle(Port p) {
		...
		final int[] aa = p.getAcceptedInterval();
	...
	public static Port[] getClosestPortPair(Port[] ports1, Port[] ports2) {
		...
		double dy;
		double d;
		for (final Port port1 : ports1) {

			pt1 = port1.getPosition();
			for (final Port port2 : ports2) {
	...
	private static List<VisualGraphObject> recursiveSearchForHitObject(VisualGraph visualGraph, Point pt,
			boolean ignoreGroups, boolean ignoreNodes, boolean ignoreEdges, List<VisualGraphObject> hitObjectsList) {

		final List<VisualGraphObject> hitObjects = visualGraph.getHitObjects(pt);

		for (int i = hitObjects.size() - 1; i >= 0; i--) {
			final VisualGraphObject vgo = hitObjects.get(i);
			if (vgo != null) {
				if (!ignoreEdges && vgo instanceof VisualEdge ) {
					hitObjectsList.add(vgo);
				} else if (vgo instanceof VisualGraph) {
					if (!ignoreGroups) {
						hitObjectsList.add(vgo);
					}
					recursiveSearchForHitObject((VisualGraph) vgo, pt, ignoreGroups, ignoreNodes, ignoreEdges,
							hitObjectsList);
	...
	/*
	 * private static GraphObjectView getFirstHitObjectInGroup( graphView
	 * visualGraph, Point pt, boolean ignoreGroups, boolean ignoreNodes, boolean
	 * ignoreEdges) { GraphObjectView hitObjects[] =
	 * visualGraph.getHitObjects(pt); for (int i = hitObjects.length - 1; i >= 0
	 * ; i--) { GraphObjectView vgo = hitObjects[i]; if(vgo != null) {
	 * if(!ignoreGroups && !ignoreNodes && !ignoreEdges ) return vgo; if(vgo
	 * instanceof EdgeView && !ignoreEdges) return vgo; else if(vgo instanceof
	 * graphView && !ignoreGroups) return vgo; else if(vgo instanceof NodeView
	 * && !ignoreNodes) return vgo; } } return ignoreGroups ||
	 * visualGraph.getLevel() == 0 ? null : visualGraph; } private static
	 * GraphObjectView getLastHitObjectInGroup( graphView visualGraph, Point pt,
	 * boolean ignoreGroups, boolean ignoreNodes, boolean ignoreEdges) {
	 * GraphObjectView hitObjects[] = visualGraph.getHitObjects(pt); for (int i
	 * = 0; i < hitObjects.length ; i++) { GraphObjectView vgo = hitObjects[i];
	 * if(vgo != null) { if(!ignoreGroups && !ignoreNodes && !ignoreEdges )
	 * return vgo; if(vgo instanceof EdgeView && !ignoreEdges) return vgo; else
	 * if(vgo instanceof graphView && !ignoreGroups) return vgo; else if(vgo
	 * instanceof NodeView && !ignoreNodes) return vgo; } } return
	 * ignoreGroups|| visualGraph.getLevel() == 0 ? null : visualGraph; }
	 */
	...
	private static boolean nested(VisualGraph graphView1, VisualGraph graphView2) {
		...
		final int l2 = graphView2.getDepth();
	...
	/**
	 * This method ensures that the edges specified by "edge" is placed in the
	 * right group. The right position for an edges is the deepest group that
	 * contains both source- and target-node.
	 * 
	 * @param edge
	 * @param sourcenode
	 * @param targetnode
	...
	public static final VisualGraph determinOptimumGraphViewForEdge(VisualGraph rootGraphView, VisualEdge edge) {
		...
		final VisualNode source = edge.getSourceNode();
		final VisualNode target = edge.getTargetNode();
	...
	public static void moveEdgeToAppropriateGraphView(VisualEdge edge) {
		...
		final VisualGraph currentView = edge.getParentGraph();
		final VisualGraph parent = currentView;
		if (parent == null) {
			return;
		}
		...
		final VisualGraph targetView = GraphInteractionUtil.determinOptimumGraphViewForEdge(topLevelView, edge);

		// System.err.println("target level for edge: " +
		// targetView.getLevel());
	...
	public static final void relocateObject(VisualGraphObject vgo, VisualGraph targetView) {
		...
		final VisualGraph currentView = vgo.getParentGraph();
	...
	private static void relocateNode(VisualNode node, VisualGraph sourceView, VisualGraph targetView) {
		...
		final Map<VisualEdge, Integer> sourcePortMap = new HashMap<VisualEdge, Integer>();
		final Map<VisualEdge, Integer> targetPortMap = new HashMap<VisualEdge, Integer>();

		final List<VisualEdge> incomingEdges = new ArrayList<VisualEdge>(node.getIncomingEdges());
		for (final VisualEdge edgeView : incomingEdges) {
			targetPortMap.put(edgeView, new Integer(edgeView.getTargetPortId()));
		}
		final List<VisualEdge> outgoingEdges = new ArrayList<VisualEdge>(node.getOutgoingEdges());
		for (final VisualEdge edgeView : outgoingEdges) {
			sourcePortMap.put(edgeView, new Integer(edgeView.getSourcePortId()));
		}
		sourceView.remove(node);
		targetView.add(node);

		for (final VisualEdge edge : outgoingEdges) {
			final Integer portID = sourcePortMap.get(edge);
			edge.connect(node, portID.intValue(), edge.getTargetNode(), edge.getTargetPortId());
			moveEdgeToAppropriateGraphView(edge);
		}
		for (final VisualEdge edge : incomingEdges) {
			final Integer portID = targetPortMap.get(edge);
	...
	private static void relocateEdge(VisualEdge edge, VisualGraph sourceView, VisualGraph targetView) {
		...
		final VisualNode sn = edge.getSourceNode();
		final int sp = edge.getSourcePortId();
		final VisualNode tn = edge.getTargetNode();
		final int tp = edge.getTargetPortId();
