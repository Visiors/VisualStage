...
import com.visiors.visualstage.tool.impl.ruler.RulerCornerButton;
...
public class RulerTool extends BaseTool {
	...
	private final RulerCornerButton rulerCornerButton;

	public RulerTool() {

		super("RULER");
		rulerCornerButton = new RulerCornerButton();
	...
	public int getPreferredCursor() {
		...
		final int hCursor = hRuler.getPreferredCursor();
		final int vCursor = vRuler.getPreferredCursor();
	...
	public void drawHints(AWTCanvas awtCanvas, DrawingContext context, boolean onTop) {
		...
		if (onTop) {
			...
			rulerCornerButton.draw(awtCanvas);
	...
	public void setSize(int size) {
		...
		hRuler.setSize(size);
		vRuler.setSize(size);
		rulerCornerButton.setSize(size);
	}
