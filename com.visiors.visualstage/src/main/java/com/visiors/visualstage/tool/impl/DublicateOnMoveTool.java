 ...
 * @version $Id: $
 */
public class DublicateOnMoveTool extends BaseTool {

	/* resizing details */
	public static final int NONE = -1;
	private static final int INTERNAL_ACTION_ID = 0xFFFF00;
	private static final int MOVE_OR_COPY_OBJECTS = DublicateOnMoveTool.INTERNAL_ACTION_ID + 1;
	private static final int MOVE_OBJECTS = DublicateOnMoveTool.INTERNAL_ACTION_ID + 2;
	...
	public DublicateOnMoveTool(String name) {
	...
	public boolean mouseReleased(Point pt, int button, int functionKey) {
		 ...
		 */
		if (hitObject != null) {
			if (manipulatoinIndex == DublicateOnMoveTool.MOVE_OBJECTS) {
				manipulatoinIndex = DublicateOnMoveTool.MOVE_OR_COPY_OBJECTS;
			}
		}
	...
	public boolean mouseMoved(Point pt, int button, int functionKey) {
		...
		manipulatoinIndex = DublicateOnMoveTool.NONE;
	...
	public boolean mouseDragged(Point pt, int button, int functionKey) {
		...
		if (hitObject != null) {
			...
			if (manipulatoinIndex == DublicateOnMoveTool.MOVE_OR_COPY_OBJECTS) { // Duplicate
				// selection
				if (isControlKeyPressed(functionKey)) {
					duplicateSelection(pt);
				}
				manipulatoinIndex = DublicateOnMoveTool.MOVE_OBJECTS;
			} else if (manipulatoinIndex == DublicateOnMoveTool.MOVE_OBJECTS) { // move
				// selected
				// objects
			...
			return manipulatoinIndex != DublicateOnMoveTool.MOVE_OBJECTS; // let other
			// like Edge
			// creation
			// do their
			// job
	...
	private void cleanUp() {
		...
		connectingSourcePort = DublicateOnMoveTool.NONE;
		connectingTargetPort = DublicateOnMoveTool.NONE;
	...
	private void trackNodesHitByActiveConnector() {
		...
		if (hitObject instanceof VisualEdge) {
			...
			if (node != null) {
				...
				if (port != DublicateOnMoveTool.NONE) {
					connectingEdge = edge;
	...
	private void reassignConnection() {
		...
		if (connectingEdge != null) {
			if (connectingSourceNode != null) {
				connectingSourceNode.illuminatePorts(false);
				if (connectingSourcePort != DublicateOnMoveTool.NONE) {
			...
			if (connectingTargetNode != null) {
				connectingTargetNode.illuminatePorts(false);
				if (connectingTargetPort != DublicateOnMoveTool.NONE) {
